...
struct ExtendedSparseMatrix_STRUCT : SparseMatrix_STRUCT{
...
/*!
  ...
  @param[in] A the known system matrix.
  @param[inout] diagonal  Vector of diagonal values (must be allocated before call to this
  function).
 */
...
inline void DeleteMatrix(SparseMatrix& A)
{
    ...
    if(A.d_elementsToSend)
        HIP_CHECK(deviceFree(A.d_elementsToSend));
    ...
    if(A.d_send_buffer)
        HIP_CHECK(deviceFree(A.d_send_buffer));

    if(A.halo_row_ind)
        HIP_CHECK(deviceFree(A.halo_row_ind));
    if(A.halo_col_ind)
        HIP_CHECK(deviceFree(A.halo_col_ind));
    if(A.halo_val)
        HIP_CHECK(deviceFree(A.halo_val));
#endif
