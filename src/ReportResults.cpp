...
void ReportResults(const SparseMatrix &A, int numberOfMgLevels,
  ...
  if (A.geom->rank ==
    ...
    double fnbytes_OptimizedProblem = 0.0f; // OptimizeProblemMemoryUse(A);
    ...
    // (times[6])); doc.get("Sparse Operations Overheads")->add("Halo exchange as
    // percentage of SpMV time", (times[6])/totalSparseMVTime*100.0);
#endif
