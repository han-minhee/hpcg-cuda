...
struct SparseMatrix_STRUCT {
  ...
  local_int_t *rowStartIndices; //  row_start index array :
  // gives the position of the starting element of each row
  ...
  double *flatMatrixValues;
  double *flatMatrixDiagonal;
  global_int_t *flatMtrxIndG;
  local_int_t *flatMtxIndL;
  ...
  /*
   the CUSPARSE triangular solver routine requires elements
  within each row to be sorted such that elements with column index smaller
  than the diagonal appear before the diagonal, and elements with column index
  larger than the diagonal appear after the diagonal. The default matrix format
  in HPCG violates this assumption in rows that are on the boundary of the local
  domain.

  In these rows the halo elements (those received from a neighbor processor)
  have column indices larger than the number of rows, but may appear before the
  diagonal because the order is inherited from the natural ordering of the
  global matrix.
  */
