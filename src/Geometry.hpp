...
struct Geometry_STRUCT {
  ...
  local_int_t
      nx; //!< Number of x-direction grid points for each local subdomain
  local_int_t
      ny; //!< Number of y-direction grid points for each local subdomain
  local_int_t
      nz;  //!< Number of z-direction grid points for each local subdomain
  ...
  int pz; //!< partition ID of z-dimension process that starts the second region
          //!< of nz values
  int npartz;     //!< Number of partitions with varying nz values
  int *partz_ids; //!< Array of partition ids of processor in z-direction where
                  //!< new value of nz starts (valid values are 1 to npz)
  local_int_t *partz_nz; //!< Array of length npartz containing the nz values
                         //!< for each partition
  int ipx; //!< Current rank's x location in the npx by npy by npz processor
           //!< grid
  int ipy; //!< Current rank's y location in the npx by npy by npz processor
           //!< grid
  int ipz; //!< Current rank's z location in the npx by npy by npz processor
           //!< grid
  ...
  global_int_t gix0; //!< Base global x index for this rank in the npx by npy by
                     //!< npz processor grid
  global_int_t giy0; //!< Base global y index for this rank in the npx by npy by
                     //!< npz processor grid
  global_int_t giz0; //!< Base global z index for this rank in the npx by npy by
                     //!< npz processor grid
};
...
inline int ComputeRankOfMatrixRow(const Geometry &geom, global_int_t index) {
  ...
  // We now permit varying values for nz for any nx-by-ny plane of MPI
  // processes. npartz is the number of different groups of nx-by-ny groups of
  // processes. partz_ids is an array of length npartz where each value
  // indicates the z process of the last process in the ith nx-by-ny group.
  // partz_nz is an array of length npartz containing the value of nz for the
  // ith group.
