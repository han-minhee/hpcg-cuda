...
*/
void GenerateGeometry(int size, int rank, int numThreads, int pz,
                      local_int_t zl, local_int_t zu, local_int_t nx,
                      local_int_t ny, local_int_t nz, int npx, int npy, int npz,
                      Geometry *geom) {
  ...
  for (int i = 0; i < npartz; ++i) {
    assert(ipartz_ids < partz_ids[i]); // Make sure that z partitioning is
                                       // consistent with computed npz value
  ...
  // These values should be defined to take into account changes in nx, ny, nz
  // values due to variable local grid sizes
